...
In this notebook we will use the resulting wrapped interferogram from `Notebook 8`, but we need to process it in the radar geometry in order to unwrap it (while in `Notebook 8` we end the whole process by performing the geocoding, just for better visualization purposes).
